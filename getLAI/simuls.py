...
def simulss(Leafpath, Soilpath, Bands=3, v_angle_min=0, v_angle_max=0, v_interval=5, s_angle_min=0, s_angle_max=0, s_interval=5, LAI_min=1, LAI_max=1, LAI_interval=1, ss_min=0, ss_max=0, ss_interval=5, vv_min=0, vv_max=0, vv_interval=5, Ha=0, Hb=1.6, nsss=0.5, Rsss=2, flag=0, vegkind=''):
    ...
    # len_k = int((LAI_max - LAI_min) / LAI_interval) + 1
    len_k = int((Decimal(str(LAI_max)) - Decimal(str(LAI_min))) / Decimal(str(LAI_interval))) + 1
    print('len_k: ' + str(len_k))
    k_ratios = np.zeros((len_k, 4))
    k_ratios_num = 0
    ...
    # 打印belta结果
    # np.savetxt('./data/belta' + str(s_angle_min) + '.txt', belta)
    ...
    for LAIa in np.arange(LAI_min, LAI_max + LAI_interval, LAI_interval):
        if LAIa > LAI_max:
                    ...
                    for i in range(v_number):
                        for j in range(s_number):
                            ...
                            for k in range(Bands):
                                # print('-----')
                                # print((1 - belta[tem1][k]) * (kcl * Rcs[k] + kg * Rgs[k]))
                                # print(belta[tem1][k])
                                # print(kcl * Rcs[k])
                                # print(kg * Rgs[k])
                                # print(S)
                                # print((1 - S) * belta[tem1][k] * kt * Rcs[k])
                                # print(S * belta[tem1][k] * kz * Rgs[k])
                                # print((1 - belta[tem1][k]) * kcnl * Rncs[k])
                                # print((1 - S) * belta[tem1][k] * kcnl_1 * Rncs[k])
                                # print(Rm[tem1][k])
                                # print('-----')
                            ...
                            # count = count + 1
                            # print(k_ratios.shape)
                            k_ratios[k_ratios_num, 0] = LAIa
                            k_ratios[k_ratios_num, 1] = k_ratio[count][9]
                            k_ratios[k_ratios_num, 2] = k_ratio[count][10]
                            k_ratios[k_ratios_num, 3] = k_ratio[count][11]
                            # k_ratios[k_ratios_num, 4] = kg
                            # k_ratios[k_ratios_num, 5] = kc
                            # k_ratios[k_ratios_num, 6] = kt
                            # k_ratios[k_ratios_num, 7] = kz
                            # k_ratios[k_ratios_num, 8] = li
                            # k_ratios[k_ratios_num, 9] = lv
                            # k_ratios[k_ratios_num, 10] = lc
                            k_ratios_num = k_ratios_num + 1
                            print('Finish! ')
            ...
            # writer.save()
            # writer = pd.ExcelWriter('./腾冲数据/res/' + str(vegkind) + '.xlsx')
            writer = pd.ExcelWriter('./lut/' + str(vegkind) + '.xlsx')
            data_df = pd.DataFrame(k_ratios)
            data_df.to_excel(writer, 'page_1', float_format='%.5f', index=False, header=False)
            writer.save()