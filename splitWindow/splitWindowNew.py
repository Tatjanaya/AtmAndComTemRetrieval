...
color: 选取颜色
"""
def splitWindow(excelFile, lutChart, atmFile, lowVapor, highVapor, ang_1, ang_2, weidu):
    # 如果不存在文件夹 则创建
    dirs = './res/'
    ...
    for i in range(1, sh.nrows):
        bianhaoLst.append(sh.cell(i, 0).value)
    ...
    for i in range(lenWV):
        if waterVapor[i] >= lowVapor and waterVapor[i] <= highVapor and bh2Int(bianhaoLst[i]) <= maxHao and bh2Int(bianhaoLst[i]) >= minHao:
            needLst.append(i)
    ...
    drawPic(BT_s8_ang_1, T_test_8_ang_1, ang_1, ang_2, lowVapor, highVapor, 'S8', ang_1, 'r', weidu)
    drawPic(BT_s8_ang_2, T_test_8_ang_2, ang_1, ang_2, lowVapor, highVapor, 'S8', ang_2, 'blue', weidu)
    drawPic(BT_s9_ang_1, T_test_9_ang_1, ang_1, ang_2, lowVapor, highVapor, 'S9', ang_1, 'violet', weidu)
    drawPic(BT_s9_ang_2, T_test_9_ang_2, ang_1, ang_2, lowVapor, highVapor, 'S9', ang_2, 'orange', weidu)
    
    print('-----')
    print("nadir: " + str(ang_1) + " oblique: " + str(ang_2) + " lowVapor: " + str(lowVapor) + " highVapor: " + str(highVapor) + " latitude: " + str(weidu))
    
    # a * x + b * (x - y) + c * (x - y)**2 + d
    f = open("./res/splitWindowRes.txt", "a+")
    f.write("拟合参数结果为(结构为a * x + b * (x - y) + c * (x - y)**2 + d): " + "\n" \
                + "参数为: " + "nadir: " + str(ang_1) + " oblique: " + str(ang_2) + " lowVapor: " + str(lowVapor) + " highVapor: " + str(highVapor) + " latitude: " + str(weidu) + "\n" \
...
def drawPic(BT, T, ang_1, ang_2, lowWV, highWV, nowBand, nowAng, color, weidu):
    ...
    plt.plot(np.linspace(240, 330, 60), np.linspace(240, 330, 60), color="green")
    plt.savefig("./res/" + str(lowWV) + "-" + str(highWV) + "_" + str(nowBand) + "_" + str(nowAng) + "°" + "_" + str(weidu) + ".png", format='png', dpi=300)
    plt.clf()
...
def bh2Int(bianhaoStr):
    loc = 0
    while(bianhaoStr[loc] == '0' and loc < len(bianhaoStr)):
        loc += 1
    return int(bianhaoStr[loc:])