...
# 主函数
def calMain(lai_lst, Tv_real, Ts_real_lst, nadir_angle, oblique_angle, ev_real, es_real, es_err_lst):
    tv_rmseLst_lst = []
    ts_rmseLst_lst = []
    # 绘图
    ...
    for es_err in es_err_lst:
        rmse_tv_lst, rmse_ts_lst = calLaiLstEvErr(lai_lst, Tv_real, Ts_real_lst, nadir_angle, oblique_angle, ev_real, es_real, es_err)
        tv_rmseLst_lst.append(rmse_tv_lst)
        ts_rmseLst_lst.append(rmse_ts_lst)
    drawSoilPic(lai_lst, ts_rmseLst_lst, es_err_lst)
    drawVegPic(lai_lst, tv_rmseLst_lst, es_err_lst)
        
def drawVegPic(laiLst, tv_rmseLst_lst, es_err_lst):
    ...
    for i in range(len(tv_rmseLst_lst)):
        ...
        es_err = es_err_lst[i]
        plt.plot(laiLst, tv_rmseLst, linestyle='-', label=str(es_err))
    plt.legend(loc='upper right')
    plt.savefig("./res/" + "土壤组分发射率-Tv.png", format='png', dpi=300, bbox_inches='tight')
    plt.clf()
    
def drawSoilPic(laiLst, ts_rmseLst_lst, es_err_lst):
    plt.xlabel("LAI")
    ...
    for i in range(len(ts_rmseLst_lst)):
        ...
        es_err = es_err_lst[i]
        plt.plot(laiLst, ts_rmseLst, linestyle='-', label=str(es_err))
    plt.legend(loc='upper right')
    plt.savefig("./res/" + "土壤组分发射率-Ts.png", format='png', dpi=300)
    plt.clf()

# 一组lai计算
def calLaiLstEvErr(lai_lst, Tv_real, Ts_real_lst, nadir_angle, oblique_angle, ev_real, es_real, es_err):
    rmse_tv_lst = []
    rmse_ts_lst = []
    for lai in lai_lst:
        rmse_tv, rmse_ts = calSoloLaiEvErr(lai, Tv_real, Ts_real_lst, nadir_angle, oblique_angle, ev_real, es_real, es_err)
...
# 单个lai计算 单个ev_err计算
# return rmse
def calSoloLaiEvErr(lai, Tv_real, Ts_real_lst, nadir_angle, oblique_angle, ev_real, es_real, es_err):
    # 没文件夹创建文件夹
    dirs = './res/'
...
es_err_lst = [0.92, 0.93, 0.95, 0.96]
calMain(lai_lst, Tv_real, Ts_real_lst, nadir_angle, oblique_angle, ev_real, es_real, es_err_lst)