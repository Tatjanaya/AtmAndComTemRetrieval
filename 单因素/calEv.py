...
def calEsErrLstRes(lai, ev_real, es_real, nadir_angle, oblique_angle, Tv_real_lst, Ts_real_lst, ev_err_lst):
    tv_rmse_res = []
    ts_rmse_res = []
    for ev_err in ev_err_lst:
        rmse_tv, rmse_ts = calEs(lai, ev_real, es_real, nadir_angle, oblique_angle, Tv_real_lst, Ts_real_lst, ev_err)
    ...
    drawPic(ev_err_lst, tv_rmse_res, ts_rmse_res, lai)

# 单个lai 单个ev_err 一整个Tv Ts lst导出的情况
# 每个lai独立绘制一张图 该函数计算一个lai 一个ev_err下的rmse（因为有多组lst）
def calEs(lai, ev_real, es_real, nadir_angle, oblique_angle, Tv_real_lst, Ts_real_lst, ev_err):
    # 没文件夹创建文件夹
    dirs = './res/'
...
# 绘制
def drawPic(ev_err_lst, tv_rmse_lst, ts_rmse_lst, lai):
    plt.xlabel("给定的植被组分发射率")
    plt.ylabel("组分温度差值")
    plt.grid()
    plt.plot(ev_err_lst, tv_rmse_lst, color="green", linestyle='--', label='植被')
    plt.plot(ev_err_lst, ts_rmse_lst, color="red", linestyle='--', label='土壤')
    plt.legend(loc='upper right')
    plt.savefig("./res/" + "ev_err" + str(lai) + "组分温度差值.png", format='png', dpi=300)
    plt.clf()
...
# ev_err = 0.92
lai = 2
# calEs(lai, ev_real, es_real, nadir_angle, oblique_angle, Tv_real, Ts_real, ev_err)
Tv_real_lst = [300, 300]
Ts_real_lst = [305, 310]
ev_err_lst = [0.95, 0.96, 0.97, 0.98, 0.99]
calEsErrLstRes(0.5, ev_real, es_real, nadir_angle, oblique_angle, Tv_real_lst, Ts_real_lst, ev_err_lst)
calEsErrLstRes(1.0, ev_real, es_real, nadir_angle, oblique_angle, Tv_real_lst, Ts_real_lst, ev_err_lst)
calEsErrLstRes(2.0, ev_real, es_real, nadir_angle, oblique_angle, Tv_real_lst, Ts_real_lst, ev_err_lst)
calEsErrLstRes(4.0, ev_real, es_real, nadir_angle, oblique_angle, Tv_real_lst, Ts_real_lst, ev_err_lst)
calEsErrLstRes(6.0, ev_real, es_real, nadir_angle, oblique_angle, Tv_real_lst, Ts_real_lst, ev_err_lst)