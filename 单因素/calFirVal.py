...
# 考虑初值对迭代解算结果的影响
# 有四个初值 Tv Ts n8_t n8_lup
# 或者也可以说三个初值 Tv Ts wv
# 单因素误差考虑一个初值的变动对于整体解算的影响

# Tv 300 
# Ts 305 310
# ev 0.98
# es 0.94
# lai 0.5 1.0 2.0 4.0 6.0 
# nadir oblique 0 55

# 基本思路 根据条件解算出TOA亮温 然后根据初值进行牛顿迭代 查看结果与初值的差异
def calTvFirVal(lai, Tv_real, Ts_real, wv_real, ev_real, es_real, nadir_angle, oblique_angle, Tv_err, tranRelaLst, upRelaLst, downRelaLst):
    # 没文件夹创建文件夹
    dirs = './res/'
    ...
    # x是初值 Tv采用错误的值进行代替
    x = [Tv_err, Ts_real, tran_8_n, lup_8_n]
    num = 4
    ...
    Newton(x, num, ecaoP4sailLst, band1, band2, c1, c2, cbtpevLst, cbtpesLst, L_bt_lst, tranRelaLst, upRelaLst, downRelaLst)

def calTsFirVal(lai, Tv_real, Ts_real, wv_real, ev_real, es_real, nadir_angle, oblique_angle, Ts_err, tranRelaLst, upRelaLst, downRelaLst):
    # 没文件夹创建文件夹
    dirs = './res/'
    if not os.path.exists(dirs):
        os.makedirs(dirs)
    ...
    # 目前将采用四个初值 Tv_err Ts_real wv_real -> tran_8_n lup_8_n
    # x是初值 Tv采用错误的值进行代替
    x = [Tv_real, Ts_err, tran_8_n, lup_8_n]
    num = 4
    ecaoP4sailLst = [ecaoP4sail_8_ang_1, ecaoP4sail_8_ang_2, ecaoP4sail_9_ang_1, ecaoP4sail_9_ang_2]
    cbtpevLst = [cbtpev_8_ang_1, cbtpev_8_ang_2, cbtpev_9_ang_1, cbtpev_9_ang_2]
    cbtpesLst = [cbtpes_8_ang_1, cbtpes_8_ang_2, cbtpes_9_ang_1, cbtpes_9_ang_2]
...
def calWvFirVal(lai, Tv_real, Ts_real, wv_real, ev_real, es_real, nadir_angle, oblique_angle, wv_err, tranRelaLst, upRelaLst, downRelaLst):
    # 没文件夹创建文件夹
    dirs = './res/'
    if not os.path.exists(dirs):
        os.makedirs(dirs)
    ...
    # x是初值 Tv采用错误的值进行代替
    tran_8_n_err, lup_8_n_err = wv2tranAndLup(wv_err)
    x = [Tv_real, Ts_real, tran_8_n_err, lup_8_n_err]
    num = 4
    ecaoP4sailLst = [ecaoP4sail_8_ang_1, ecaoP4sail_8_ang_2, ecaoP4sail_9_ang_1, ecaoP4sail_9_ang_2]
    cbtpevLst = [cbtpev_8_ang_1, cbtpev_8_ang_2, cbtpev_9_ang_1, cbtpev_9_ang_2]
    cbtpesLst = [cbtpes_8_ang_1, cbtpes_8_ang_2, cbtpes_9_ang_1, cbtpes_9_ang_2]
...
# wv_err = 3.38
# 这部分可以说明
# 如果单一的组分温度初值产生偏差 那么对最后迭代结果不会有太大的影响
# 水汽含量需要保持在一定范围内 才会对最后迭代结果产生较小的影响 否则无法迭代出合理的结果

# calTvFirVal(lai, Tv_real, Ts_real, wv_real, ev_real, es_real, nadir_angle, oblique_angle, Tv_err, tranRelaLst, upRelaLst, downRelaLst)
# calTsFirVal(lai, Tv_real, Ts_real, wv_real, ev_real, es_real, nadir_angle, oblique_angle, Ts_err, tranRelaLst, upRelaLst, downRelaLst)
# calWvFirVal(lai, Tv_real, Ts_real, wv_real, ev_real, es_real, nadir_angle, oblique_angle, wv_err, tranRelaLst, upRelaLst, downRelaLst)
res = calTvTsWvFirVal(lai, Tv_real, Ts_real, wv_real, ev_real, es_real, nadir_angle, oblique_angle, Tv_err, Ts_err, wv_err, tranRelaLst, upRelaLst, downRelaLst)
# 如果迭代结果符合规律 则打出目前的条件
